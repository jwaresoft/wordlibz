 ...
 * @returns
 */
function indexValuesToReplace(
  tagList: Array<PosTaggedToken>,
  ...
  return tagList.map(
    (
      posTaggedItem: PosTaggedToken,
      index: number,
    ): IndexedAndLabeledPOSItem => {
      const indexToAdd = isProperNoun(posTaggedItem.pos)
        ? handleProperNouns(posTaggedItem, index)
        : [index];
