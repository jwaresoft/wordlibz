  ...
  describe("resetUserValues()", () => {
    it("should remove additional keys from UserPOSItem type", () => {
      const testSentencesTagged = tagExtract(
        "What is the biggest hat you own?",
      );
      ...
      userReadyValues.forEach((element, index) => {
        ...
        expect(element.originalIndex).toEqual(
          taggedWithIndex[index].originalIndex,
        );
        expect(element.pos).toEqual(taggedWithIndex[index].pos);
        expect(element.value).toEqual("");

        expect((element as unknown as IndexedAndLabeledPOSItem).lemma).toBe(
          undefined,
        );
        expect((element as unknown as IndexedAndLabeledPOSItem).normal).toBe(
          undefined,
        );
        expect((element as unknown as IndexedAndLabeledPOSItem).tag).toBe(
          undefined,
        );
      });
